...
int lookup(qname,origin,qclass,qtype,zones,cache,ncache,mba,now,
    ...
    struct btnode *zones, *cache;	/* Pointers at roots of database */
	...
	if(result != NULL) {
	    ...
	    *answer = mak_domain(1);
		...
		buginf("lookup","cname \"%s\" found, new ntags=%d",
			namtoa(rrtext,cname_ptr->rdata[CNAME_CNAME]),ntags);
	    }
	    ntags = cname_idx
		  + namexp(cname_ptr->rdata[CNAME_CNAME],vector+cname_idx);
