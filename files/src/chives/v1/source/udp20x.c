 ...
 */
extern void bughlt(),bugchk(),buginf();
...
/* System call to use */
#if	monsymdefined("GTDOM%")
#define GTDOM_ monsym("GTDOM%")
#elif	monsymdefined("GTHST%")
#define GTDOM_ monsym("GTHST%")
#else
#error	GTHST%/GTDOM% not defined!
#endif
    ...
    int foreign;
{
    ...
    if((ac[2] = foreign) != 0 && (ac[1] = monsym(".GTDLA"), jsys(GTDOM_,ac)))
    ...
    if((ac[1] = monsym(".GTHSZ"), jsys(GTDOM_,ac)))
    ...
    int count,list[];
{
    ...
    ac[2] = (int) list;
    ac[3] = count;
    if(jsys(GTDOM_,ac))			/* if we did it, flush out */
    ...
    int lhost,lport;
{
	...
	udpiqh = udp_jsys(monsym("ASNIQ%"),(int *)&argblk,0,
    ...
    int opcode, *argblk, argnum, size;
{
	...
	switch(opcode) {
	    case monsym("ASNIQ%"):	/* ASNIQ% jsys */
	    case monsym("SNDIN%"):	/* SNDIN% jsys */
    ...
    char *p;
{
    ...
    if(LOGFLG)
	udp_logpkt("send",p);
    return(udp_jsys(monsym("SNDIN%"),(int *)p,udpiqh,*(int *)p));
}
    ...
    int waitp;				/* should we block? */
{
    ...
    *(int *)p = (bsize+WORD_SIZE-1)/WORD_SIZE;
    ...
    err = udp_jsys(monsym("RCVIN%"),(int *)p,waitp,*(int *)p);
    if(LOGFLG && err != 0)
	udp_logpkt("recv",p);
    return(err);
}
