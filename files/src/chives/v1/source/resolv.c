...
static int chkrun = 0;			/* Make running checkpoints (NIY) */
...
int config()
{
    ...
    if(lsearch == NULL)			/* paranoia, but cheap */
	lsearch = nullsearch;
    ...
    struct rr *ns_list;
{
    ...
    if(ns_list == NULL) {		/* No delegation at all? */
...
do_answer()
{
    ...
    for(h = NULL; udp_recv(packet,BIG_UDP_PACKET,0); h = NULL) {

	h = DOM_HEADER(UDP_DATA(packet));
	if(UDP_HEADER(packet)->ck != 0 && udp_chksum(packet) != 0)
	...
	++busy;
	seq = h->id;
	qpeek(DOM_DATA(UDP_DATA(packet)),&qname,&qclass,&qtype);
	for(q = queue; q != NULL; q = q->next)
	    if(seq == q->seq && qclass == q->class
			     && qtype == q->type
			     && !namcmp(qname,q->cqname))
		break;
	if(DBGFLG(DBG_LOG|DBG_RESOLV)) {
		...
		rrhtoa(rrtext,qname,qclass,qtype));
	}
	if(q == NULL)
	 ...
	 */
	if((rrlist = rpkt(UDP_DATA(packet),UDP_HEADER(packet)->ln)) == NULL)
    ...
    if(ercode != UE_OK) {
	if(DBGFLG(DBG_RESOLV))
	    buginf("do_answer","ignoring this packet");
