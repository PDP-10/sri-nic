...
extern int namcmp(), namlen(), namcnt(), namexp(), namkin(), tagcmp();
extern char *namcpy(), *namcat(), *namimp(), *namtoa(), *vnamtoa();
extern char *inatoa(), *wkstoa(), *rrtoa(), *rrhtoa();
extern int atoina();

/* AVL tree routines */
int avladd(), avldel(), avlcnt(), avlwalk();
void avltab(), avlkil();
struct btnode *avlook();

...
extern union rdatom	*mak_rdatom();
extern struct rr	*mak_rr();
extern void		 kil_rr();
extern struct domain	*mak_domain();
extern void		 kil_domain();
extern struct btnode	*mak_btnode();
extern void		 kil_btnode();
extern struct zone	*mak_zone();
extern void		 kil_zone();

/* Cross between mak() and bcopy(). */
extern char		*bcons();

/* Temporary kludge (ha ha). */
extern struct rr	*rrcons();

...
int usr_init(), usr_send();
struct query_block *usr_recv();
void usr_rxmt(), usr_fini();

/* Packet parsing routine. */
extern struct domain *rpkt();
...
/* Bug handling. */
extern int bugini();
extern void buginf(),bugchk(),bughlt(),bugfls();
