...
static char7 bugfnm[60] = "???";
static char7 dmpnam[40*4];
static int bugrtm, bugctm, bugjer, bugfkx;
static int bugac_[020],  bugvec[2];
static int chkacs[020], chkvec[2];
static int bugpc[6], bugrcm, bugrwm, bugrlm, bugaos = -1, bugchn;
...
static ss_arg = FLD(-NPAGES,monsym("SS%NNP"))
...
repeat 44,<
	<<$level*10000000000>+<$$SECT,,chnvec+$rpcnt>>
	$rpcnt==$rpcnt+1
>
    ...
    int *dumpp;
{
    ...
    sprintf(bugfnm,"DOMAIN:%s.CRASH.-1",bugxjn);
    if(!psiini())
...
static int panics = BIT(monsym(".ICPOV"))   /* Pushdown overflow */
...

static void bugint()			/* called at PSI level */
{					/* NB: not reentrant! */
#asm
	aose	bugaos			/* Are we looping? */
...

void buginf(va_alist)
    va_dcl /* ; */
{
    char *module,*format;
    va_list vargp;
    va_start(vargp);
    module = va_arg(vargp,char *);
    format = va_arg(vargp,char *);
...
{
#asm
	movei	1,.fhslf		/* Get most recent error */
    ...
    va_dcl /* ; */
{
	...
	va_start(vargp);		/* Redo this in gory detail for */
	module = va_arg(vargp,char *);	/* varargs' benefit */
	format = va_arg(vargp,char *);
	bugtyo("?BUGHLT",module,format,&vargp);
	va_end(vargp);
	...
	r_i_p_();			/* Commit honorable suicide */
}

...
static void chklog(truename,status)
