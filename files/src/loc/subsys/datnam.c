/* [SRI-NIC]PS:<NIC-WORK>DATNAM.C.8, 13-Jan-90 23:34:53, Edit by VIVIAN */
/* Change the output filename to be of form NAME.EXT-DATE.  This is mostly */
/* good for files where there is more than one extension with the same     */
/* first portion.                                                          */
/***************************************************************************
  DATNAM.C - File renamer
  ...
  Usage:
    datnam  -format source_file [dest_dir]
...
char *argv[];
{
    ...
    if (argc < 3 || argc >= 5 || argv[1][0] != '-') {
        fputs("Usage: datnam  -{y | m | d | yy | ym | yd} source [dest_dir]\n", stderr);
        exit(1);
    }
