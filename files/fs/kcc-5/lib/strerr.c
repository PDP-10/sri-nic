...
int errnum;
{
    ...
    */
    if (_ERRNO_UNX_FIRST <= errnum && errnum <= _ERRNO_UNX_LAST
      && (errnum - _ERRNO_UNX_FIRST) < sys_nerr)
	return sys_errlist[(errnum - _ERRNO_UNX_FIRST)+1];
#if SYS_T20+SYS_10X
...
baderr(num)
{
    static char badmsg[40];	/* 16 plus room for 24 octal digits (!) */
    sprintf(badmsg, "Unknown error (%#o)", num);
    return badmsg;
}
