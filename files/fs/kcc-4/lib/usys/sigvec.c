...
#ifndef NULL			/* Eventually get this from stddef.h */
#define NULL 0
#endif

#if !(SYS_T20)
#error sigvec() not supported for this system.
#else
...
static void _siginit(), enable_ints(), disable_ints();
...
static int *aintflgs;			/* Addr of int PC flags */
static int chvign, chvigx, chvhlt, chvmisc,	/* Special chntab vectors */
	chvili;
...
static int sigostk;			/* Saved stk ptr for debugging */
...
int sig, chn, cha;
{
    ...
    /* Check first for hacking CHNmisc - the misc-sig channel.
    ** This gets activated only when some signal first needs it,
    ** and thereafter is never deactivated.
    */
    if (chn == CHNmisc) {	/* Misc-sig channel is special */
	if (cha != CHA_NOACT && chntab[CHNmisc] == 0) {
	    chntab[CHNmisc] = chvmisc;	/* Set up special PSI dispatch */
	    chn_aic(chnmask(CHNmisc));	/* then activate the channel! */
	}
	return;
    }
...
#asm
	...
	cis%		/* Clear it to be completely safe */

	xmovei 1,levpcs+<<PSILEV-1>*2>	/* Find address of PC and flags */
	movem 1,aintflgs	/* Store flags address */
	cail 17,
	 addi 1,1		/* If extended, PC is stored in next wd */
	movem 1,.intpca		/* Store PC address */

	movsi 5,-44		/* For all channels */
	move 1,[JSR PSISIG]	/* Standard secondary dispatch for sig chns */
	movem 1,chnxct(5)	/* Store that as 1st instr to xct */
	aobjn 5,.-1

	/* Build chntab vector dispatch words */
	movsi 1,<PSILEV>	/* Get PSI level to use */
	cail 17,
	 lsh 1,14		/* Extended, must shift higher up */

	xmovei 2,psich0		/* Set up special dispatch for CHN0 */
	ior 2,1
	movem 2,chntab+0	/* Always store it immediately in chntab! */
	xmovei 2,psimsc		/* Set up special dispatch for CHNmisc */
	ior 2,1
	movem 2,chvmisc
	xmovei 2,psiign		/* Set up special dispatch for CHA_ACTIGN */
	ior 2,1
	movem 2,chvign
	xmovei 2,psiigx		/* Another alternative one for CHA_ACTIGN */
	ior 2,1
	movem 2,chvigx
	xmovei 2,psihlt		/* Set up special dispatch for CHA_ACTHALT */
	ior 2,1
	movem 2,chvhlt
	xmovei 2,psiili		/* Set up special dispatch for .ICILI handler*/
	ior 2,1
	movem 2,chvili

	jumpge 17,psiin5	/* Jump if extended */

	/* Initialize for single-section */
	movei 1,.fhslf		/* arg 1: fork handle */
	move 2,[levtab,,chntab]	/* arg 2: level & channel table addrs */
	sir%
	jrst psiin7

	/* Initialize for multi-section (extended) */
psiin5:	movei 1,.fhslf		/* arg 1: fork handle */
	xmovei 2,4		/* arg 2: addr of arg blk */
	movei 4,3		/* argblk/ length (3)  */
	xmovei 5,levtab		/*         levtab addr */
	xmovei 6,chntab		/*         chntab addr */
	xsir%			/* Extended SIR */

psiin7:
#endasm
	chn_aic(chnmask(CHN0));	/* Activate the special CHN0 channel */
...
psihlt:	haltf%		/* Special handler to HALTF% then continue */
psiign:	debrk%		/* Special handler to ignore most interrupts. */
psiigx:	movem 1,savac1	/* Special handler to ignore certain interrupts, */
...
psiili:	dmovem 1,savac1	/* Special handler to fix up .ICILI PC flag */
...
piret5:	movei 14,015		/* ac 1: count (13 acs) */
	xmovei 15,sc.acs(16)	/* ac 2: source */
	setz 16,		/* ac 3: dest */
	extend 14,[xblt]	/* Restore ACs 0-13 */
	move 16,15		/* Get back ptr to remaining ACs */
	dmove 14,(16)		/* Restore 14-15 */
	dmove 16,2(16)		/* Restore 16-17 */
	debrk%			/* Return! */

...
psimsc:	skiple .sigusys		/* Are we in USYS code? */
	...
	andcm 1,csgmsk+CHNmisc	/* and which of these map into this channel */
...
pimsc3:	skipl 2,sigact(1)	/* Find SA_ action for this signal */
	...
	/* Anything else (SA_TRM, SA_DMP, SA_STP) comes here. */
...
sigrt5:	movei 14,015		/* Move only 13 ACs */
	xmovei 15,sc.acs(16)	/* Source is start of saved ACs */
	setz 16,		/* Dest is AC 0 */
	extend 14,[xblt]	/* Do the XBLT */
	dmove 14,<sc.acs+14>-<scsiz-1>(17)	/* Restore 14 and 15 */
	move  16,<sc.acs+16>-<scsiz-1>(17)	/* Restore 16 */
...
psich0:	movem 1,savac1		/* Save AC just in case we die here. */
...
int pid, sig;
{
	...
	if (pid == _pidpar)
	    ...
	    acs[1] = 0400000 + ((pid >> 9) & 0777);
	    /* Note that SIGKILL can be done with HFORK% for child procs */
	    if (sig == SIGKILL) {
		if (jsys(HFORK, acs) <= 0) USYS_RETERR(EACCES);
		USYS_RET(0);
	    }
...
int sig;
{
    ...
    if (bit & snocatch) {	/* Is this one of the uncatchable signals? */
    ...
    if (bit & csgmsk[CHNmisc]) {	/* Is this one of the misc signals? */
	...
	*/
	static void addmask();
	addmask(bit, chnmask(CHNmisc));	/* Add to pending sig/chn masks */
	(void)USYS_END();		/* Trigger the interrupt now! */
	return;
    }
    /* Signal with an assigned channel, find the channel bit to trigger */
    acs[2] = _sigcmask[sig] & (-_sigcmask[sig]);	/* Find high chn bit */
    if (acs[2] == 0) abort();			/* Should always have a bit! */
    ...
    */
    (void)USYS_END();		/* Leave USYS mode */
    acs[1] = _FHSLF;
    jsys(IIC, acs);		/* Trigger the interrupt! */
...
***			REFERENCE ONLY
**/
#if 0		/* Gigantic commented-out block */
...
#endif	/* End of Gigantic commented-out block */
