...
SSTART:	RESET%			/* Initialize the world monitor-wise. */
	...
	MOVE A,.JBHSP		/* Get page # of start of high seg */
	...
	ADDI	A,PG$SIZ-1
	ANDCMI	A,PG$MSK	/* Round up to page boundary */
	MOVEM	A,.END		/* Save it as the END */
	MOVEM	A,.ETEXT	/* Same as END */
	MOVEI	A,770000	/* Set this as allocation limit, so that */
