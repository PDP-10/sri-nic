...
static int _tzloc();		/* Get local timezone in secs */
...
static char tzbuf[10];		/* For "GMT+hh:ss\0" */
...
int mwest, dstt;
{
    ...
    *++cp = ':';
    *++cp = '0' + (rem / 10);	/* tens digit of remainder (sec) */
    *++cp = '0' + (rem % 10);	/* ones digit of remainder (sec) */
    *++cp = '\0';		/* Polish off */
    return tzbuf;
}

...
static int _tlzone;		/* Local timezone */
