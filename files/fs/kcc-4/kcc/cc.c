...
static void cfile(), showcpu(), setfix(), setcpu();
static void coptimize(), cdebug(), ctargmach(), cportlev(), cverbose();
static int mainsymp();

static char mainname[_FILSIZ] = {0};	/* Name of module containing "main" */
static char *savname = NULL;		/* Pointer to desired -o filename */
static int iflag = 0;			/* Whether -i switch seen */

...
char **argv;
{
	...
	if (argc > 1) {
	    ...
	    fprintf(stderr, "kcc: usage: cc [-x -y -z] filename\n");
	    exit(0);
	}
    }

    /* Process command line.  First scan for all switches */
	...
	runnext(nextprog);	/* go run it.  Never returns! */
		...
		|| iflag)		/* or gave -i flag, */
		argv[0] = "-i";		/* preload special library! */
	    else argv[0] = NULL;	/* Else just zap 1st arg. */
...
struct flagent cplevtab[] = {
	...
	"ansi",	&clevel, CLEV_ANSI,	/* Parse ANSI implementation */
	NULL,	NULL,	NULL
};
