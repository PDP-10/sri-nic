...
EXT int line, page,	/* position in input file */
...
*/
extern NODE nodes[];	/* Parse tree node table (CCSTMT, CCDUMP, CCDATA) */
EXT int maxnode;	/* maximum node used (CCDECL, CCSTMT, CCGEN) */
EXT SYMBOL *curfn;		/* name of current function */
EXT int curfnloc, curfnnew;	/* where in file it started (CCERR, CCDECL) */
EXT int maxauto;	/* size of auto vars (CCGEN, CCDECL) */
EXT int stackrefs;	/* whether this fn addresses locals */
EXT int stkgoto;	/* Whether this fn expects a non-local goto */
