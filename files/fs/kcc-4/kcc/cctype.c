...
TYPE *to, *frm;
{
	...
	/* Hack for now - convert general pointer-pointer conversion into
	** some more specific type of conversion.
	** Also provide some other hacks so CCGEN2 doesnt have to change yet.
	*/
	case CAST_AR_PA:
	case CAST_PT_PT:
	    break;
...
TYPE *t;
{
	...
	n = convunary(n);		/* Special hack */

    if (t == n->Ntype && optpar)	/* May be a trivial cast */
	return n;
...
NODE *n;
{
    ...
    /* OK to convert any arith type to any arith type */
    if (tisarith(lt) && tisarith(rt)) {
	return convcast(lt, n);
    }

    /* OK to convert integer 0 to any pointer type */
    if (lt->Tspec == TS_PTR && niszero(n)) {
	return convcast(lt, n);
    }

    /* OK to convert "array of T" to "pointer to T" */
    if (rt->Tspec == TS_ARRAY && lt->Tspec == TS_PTR
	&& rt->Tsubt == lt->Tsubt) {
	return convunary(n);		/* Just apply usual conv */
    }

    /* OK to convert "function" to "pointer to function" */
    if (rt->Tspec == TS_FUNCT && lt->Tspec == TS_PTR
	&& lt->Tsubt->Tspec == TS_FUNCT) {
	return convunary(n);		/* Just apply usual conv */
    }

    /* Technically nothing else is OK */
    cop = cast_op(lt, rt);		/* See if we can cast anyway */
					/* (If not, cast_op complains) */
    if (cop != CAST_ILL) {		/* If yes, give warning. */
	if (cop == CAST_PT_PT		/* If pointer-to-pointer, */
	    && clevkcc			/* Check for special KCC hack */
	    && n->Nop == N_SCONST	/* Casting a str literal */
	    && tischar(lt->Tsubt)) {	/* to another kind of char ptr? */
		n->Ntype = lt;		/* Yes, just change type of const! */
		return n;		/* And code gen will win. */
	}
	/* Otherwise, do cast but give warning. */
	warn(EGEN,"Implicit conversion - %s to %s",
		tsnames[rt->Tspec], tsnames[lt->Tspec]);
    }
    n = defnode(N2, N_CAST, lt, 0, n);	/* Set up the cast */
    n->Ncast = cop;
    return n;
}

...
#if 0
	    n = defnode(N2, N_ADDR,
			findtype(TS_PTR, n->Ntype->Tsubt), 0, n);
	    if (!(n->Nleft->Nflag & NF_GLOBAL)) { /* If obj has local extent */
#else
...
#if 0
    if (n->Nflag & NF_ARRFCONV) {	/* Need to undo conversion? */
	if (n->Nop != N_ADDR)
	    error(EGEN,"Internal error - no N_ADDR for fncall");
	n = n->Nleft;			/* Dike out the N_ADDR */
	if (top) {			/* Fix up expr list if one */
	    for(np = top; ; np = np->Nright) {
		np->Ntype = n->Ntype;
		if (np->Nright->Nop != N_EXPRLIST)
		    break;
	    }
	    np->Nright = n;		/* Replace the N_ADDR with Q_IDENT */
	    return top;
	}
    }
#endif
