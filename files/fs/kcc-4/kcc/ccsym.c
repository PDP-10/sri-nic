...
** pointer then the "size" is size in words.  If the pointer is a char
** pointer then its "size" is expressed in terms of chars (9-bit bytes).
...
TYPE *t;
{
    ...
    */
	...
	size *= (TGSIZ_WORD / elembsiz(t));	/* Multiply by # bytes/wd */
    return size;
}

/* SIZEARRAY - returns number of elements in array.
...
/* ELEMBSIZ - Returns size in bits of an element (in an array, or pointed to)
