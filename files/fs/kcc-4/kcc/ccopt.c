...
void foldinc(), foldplus(), foldbyte(), foldboth();
...
**
** Often a for(;;) loop will have an P_AOS (and possibly a test) at the bottom.
** The variable is likely to be used in the body of the loop, so for simple
** loops we can merge the P_AOS in with the previous P_MOVE of the same variable
...
PCODE *p;
{
    ...
    q = p;
    while ((q = before (q)) != NULL) switch (q->Ptype & PTF_ADRMODE) {
    case PTA_REGIS:
	safeindex[q->Preg] = 0;
	safeindex[q->Pr2] = 0;
	switch (rchange(q->Pop)) {
	    case PRC_DSET:		/* Does op change register pair? */
	    case PRC_DCHG:
	    case PRC_DCHG_RSAME:
	    case PRC_UNKNOWN:
		safeindex[q->Preg + 1] = 0;	/* Yes, account for it. */
		safeindex[q->Pr2 + 1] = 0;
		break;
	    case PRC_RSAME:			/* Harmless instr */
	    case PRC_RSET:
	    case PRC_RCHG:
	    case PRC_DSAME:
		break;
	    default:
		error(EINT,"bad PRC_ val in foldinc");
		break;
	}
	break;
	...
	case P_MOVE:
		...
		do {
		    ...
		    if ((b->Ptype&PTF_ADRMODE) == PTA_MINDEXED
		      && b->Pindex == q->Preg)
			b->Poffset += i;
		    else if (b->Ptype == PTV_IMMED && b->Preg == q->Preg &&
			     b->Pop == P_IMUL) i *= b->Pvalue;
		} while (b != p);
		i = p->Preg;
	...
	/* Ops that change a register pair */
	...
	case P_SETZ+POF_BOTH:	case P_SETO+POF_BOTH:	case P_SETCM+POF_BOTH:
	    if (q->Preg == p->Pindex || sameaddr (p, q, 0) || alias (p, q, 0))
		return;
	    safeindex [q->Preg] = 0;
	    continue;
	...
	*/
    ...
    default:
	error(EINT,"illegal Ptype seen in foldinc()");
	return;
    }
}

...
int reg;
{
	...
	case PTA_REGIS:			/* register to register */
	    if (p->Pr2 == reg)		/* R used as E */
		break;
	    return 0;
	case PTA_MINDEXED:		/* addr+offset(index) */
	    if (p->Pindex == reg)	/* R used as index reg */
		break;
	    if (!p->Pptr && !p->Pindex && p->Poffset == reg)
		break;			/* R used as E (in illegal way) */
	    return 0;
	case PTA_BYTEPOINT:		/* [bsize,,addr+offset(index)] */
	    if (p->Pindex == reg)	/* R used as index reg */
		break;
	    if (!p->Pindex && !p->Pptr && p->Poffset == reg)
		break;			/* R used as E (in illegal way) */
	    return 0;
	...
	case PTA_DCONST2:
	    return 0;
	default:
	    error(EINT,"unknown pcode adrmode in rinaddr()");
	    return 0;
	}
    return 1;		/* If we get here, register is used by addr! */
}
...
int r;
{
	...
	case PRC_DCHG:
	case PRC_DCHG_RSAME:
	    if (p->Preg + 1 == r)	/* Change second of pair? */
		return 1;
					/* Else drop through */
	case PRC_RSET:
	case PRC_RCHG:
	case PRC_RCHG_DSAME:
	    if (p->Preg == r)		/* Change reg? */
		return 1;
	    break;
    ...
    /* Here, the reg isn't changed by being in AC field of op.
    ** Try to see if it can be changed as a memory operand.
    */
    switch (p->Ptype & PTF_ADRMODE) {
	case PTA_REGIS:
	    if (p->Pr2 != r)
		return 0;
	    break;
	case PTA_MINDEXED:	/* Very unlikely but just in case */
	case PTA_BYTEPOINT:
	    if (p->Pindex || p->Pptr || p->Poffset != r)
		return 0;
	    break;
    }

    /* Memory operand refers to this register.  Try to find out if this
    ** op changes memory.
    */
    if (p->Pop & POF_BOTH)	/* If BOTH flag set, */
	return 1;		/* assume something funny going on */
    if (popflg[p->Pop & POF_OPCODE] & PF_MEMCHG)	/* Can op change mem?*/
	return 1;
    ...
    return 0;	/* Not known to be a mem changing op, assume OK */
}

