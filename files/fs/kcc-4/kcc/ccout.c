...
static int binexp(), fltpow2(), outdecl(), bigfloat(), obplh();
...
int mainf;			/* TRUE if main() was defined in file */
{
    ...
    if (mainf) {		/* main module needs starting addr */
	outstr("$$STRT: JRST $START\n");	/* Stupid crock needed */
	outstr("\tJRST $START+1\n");		/* because FAIL requires */
	++crtref[CRT_START];	/* END's arg to be def'd in current module! */
    }
...
int flag;		/* 0 = definitions are INTERNed, 1 = ENTRYed */
{
	...
	if (s->Sclass == SC_EXTERN || s->Sclass == SC_AEXTERN) {
		...
		if (s->Srefs == 0		/* If never used, don't ask. */
		  || tgasm == TGASM_MIDAS)	/* MIDAS needn't even ask. */
			outc(';');		/* Comment out. */
		outstr("\tEXTERN\t");
	    }
	    outid (s->Sname);
...
char *s;
{
    ...
    outid(s);			/* Output the actual label name */
    if (s[0] == '$')		/* Local label? */
...
ahmacs()
{
    ...
    /* Make request for standard C library, and */
    /* Specify version of compiler and library used with this module */
    sprintf(cp,"\t.REQUEST %sc%s\n\t$$CVER==<%o,,%o>\n\t%s $$CVER\n",
		libprefix, libpostfix, cvercode, cverlib,
