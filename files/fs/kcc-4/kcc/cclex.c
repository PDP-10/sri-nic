...
#define putcpool(c) (--cpleft <= 0 ? (int)fatal(ECPOOL) : *cpool++ = (c))

/* ---------------------------------------- */
...
{
    tokstack = 0;
    cpool = _cpool;		/* Init pointer into char pool */
...
SYMBOL *s;
{
	...
	fatal(EINT, "Internal token stack depth exceeded");
