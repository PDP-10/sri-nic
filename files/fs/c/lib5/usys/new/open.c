...
/*
 *	Allocate per-FD storage.  ordinarily these would be combined into
 ...
 *	FDs can be numbered from 0 to OPEN_MAX-1 inclusive.
 *
 *	UFXs (Unix File Indexes) are from 1 to OPEN_MAX inclusive, as it
 *	is easier to maintain the mapping table this way (0 means the FD
 *	is unassigned).
 *
 *	The arrays are initialized for stdin, stdout, and stderr; actual
 *	runtime startup may modify those values.
 */
...
int _uioch[OPEN_MAX+1] =	/* JFN for channel */
	{ 0,
#if SYS_T20
	_PRIIN, _PRIOU, _CTTRM
#endif
	};
int _uiotype[OPEN_MAX+1] =	/* device type  */
	{ 0, _DVTTY, _DVTTY, _DVTTY };
int _uioflgs[OPEN_MAX+1] =	/* flag bits (see UIO.H) */
	{ 0, _UIO_READ|_UIO_CONVERTED,
	 _UIO_WRITE|_UIO_CONVERTED,
	 _UIO_WRITE|_UIO_CONVERTED };
...
int _uioszb[OPEN_MAX+1];		/* size in bytes of file */
int _uioszp[OPEN_MAX+1];		/* size in pages of file */
int _uiolfp[OPEN_MAX+1];		/* Last File Page read, or -1 */
int _uiobi[OPEN_MAX+1];			/* Buffer Index of current map buf */
int _uioiu[OPEN_MAX+1];			/* # of buffers In Use */
int _uiopm[OPEN_MAX+1][N_BUFFERS];	/* page#s in our memory of file maps */
int _uiofp[OPEN_MAX+1][N_BUFFERS];	/* corresponding page#s in the file */
#endif
...
int flags, mode;
{
	...
	return -1;		/* Failed to get FD or UFX slot */
 ...
 */
	...
	return -1;		/* Failed to get JFN for file */
 ...
 */
	...
	return -1;		/* and lose-return */
    }
/*
 *	if this is a disk file:
 *
 *	get the size of the file in pages (_uioszp) and bytes (_uioszb).
 *	if the byte count is zero, assume it's a binary file, so figure
 *	it as the # of bytes that will fit in the file's # of pages.
 */
    if (_uiotype[ufx] == _DVDSK) {
	_uioszp[ufx] = _gtfdb(jfn,_FBBYV) & RH;	/* # of pages in file */
	if (!(bytes = _gtfdb(jfn, _FBSIZ)))	/* size in bytes */
	    bytes= _uioszp[ufx] * PAGE_SIZE_INT * (INT_SIZE_BITS/_uiobsz[ufx]);
	_uioszb[ufx] = bytes;			/* # of bytes in file */
/*
 *	then initialize the disk-map stuff, and	pre-read the first bufferful.
 */
	_uiobi[ufx] = -1;			/* no last file page or */
	for (i = 0; i < N_BUFFERS; i++)		/* current buffer */
	    _uiopm[ufx, i] = _uiofp[ufx, i] = -1;
	_uioiu[ufx] = 0;			/* no buffers in use */
	_uiolfp[ufx] = 0;			/* start at head of file */
	_uioflgs[ufx] |= _UIO_NEW_POS;		/* at a new place in file. */
	_uiomapbuf(ufx);			/* pre-read first pages */
    }
#else
...
int job_wide_flag;
{
	...
	if (p = strchr(cp, '<')) {		/* if got a directory */
	    ...
	    *token = (job_wide_flag) ? '*' : '$'; /* right side means sys */
	    ...
	    if (!strSUB(tried, token)) {	/* if haven't tried this */
 ...
 *			A size of 0 or 36 becomes 9.  Any other size
 *			is simply used, with probably nonsense results.
 */
 ...
 */
	...
	|| (!(flags & O_UNCONVERTED) && !(flags&O_BINARY) && byte_size == 7))
