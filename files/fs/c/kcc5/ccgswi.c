...
/* Imported functions */
extern VREG *genexpr();
extern void codlabel(), codgolab();	/* CCCODE */
extern SYMBOL *newlabel();		/* CCSYM */

/* Internal functions */
static void casejump();
static int countcases(), labcomp(), unique();
...
SYMBOL **deflab;
{
    ...
    for (; n != NULL; n = n->Nright) {
      switch (n->Nop) {
	default:
	    error(EINTNOD, "Non-case node on switch list", n);
	    return -1;

	case Q_CASE:
	    if (ncases >= MAXCASE) {
		error(EINT,"More than %d case statements (%d)",MAXCASE,ncases);
		return -1;
	    }
...
SYMBOL **deflab;
{
	...
	case Q_CASE:
	    val = n->Nxfint;
	    if (ncase >= MAXCASE) {
		fatal(EINT,"More than %d case statements (%d)",MAXCASE,ncase);
		return 0;
	    }
...
static void
casejump (reg, caselab, ncase, ltable, xlabel)
VREG *reg;
struct lablist caselab[];
