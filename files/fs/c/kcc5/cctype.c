...
TYPE *newtyp;
{
	...
	if (n->Nleft->Nop == N_CAST		/* If casting a cast that's */
	  && (n->Nleft->Nflag & NF_USERCAST)==0) {	/* also implicit */
	    /* then just change the cast operation to the right thing. */
	    n->Nleft->Ncast = cast_op(newtyp, n->Nleft->Nleft->Ntype);
	    n->Nleft->Ntype = newtyp;
    ...
    /* Check right-hand side, same as above. */
    if (n->Nright->Ntype != newtyp) {
	if (n->Nright->Nop == N_CAST		/* If casting a cast that's */
	  && (n->Nright->Nflag & NF_USERCAST)==0) {	/* also implicit */
	    /* then just change the cast operation to the right thing. */
	    n->Nright->Ncast = cast_op(newtyp, n->Nright->Nleft->Ntype);
	    n->Nright->Ntype = newtyp;
	} else {				/* Casting something else */
