...
**		All changes after v.116, 29-Apr-1988
**	(c) Copyright Ken Harrenstien, SRI International 1985, 1986
**		All changes after v.17, 8-Aug-1985
**
**	Original version (C) 1981  K. Chen
*/
#define NEW 3	/* temporary */
...
NODE *evalexpr();
NODE *evaldiscard();
int sideffp();		/* Used here and CCGEN2 */
...
NODE *e;
{
    ...
    */
    ...
    default:
	error(EINTNOD,"eval: bad expr op", e);
...
NODE *e;
{
	...
	if (list->Nop == N_EXPRLIST)
	    ...
	    error(EINTNOD, "evalfun: bad arglist", list);
	    break;
	}
    }
    return e;
...
NODE *e;
{
    ...
    default:
	error(EINTNOD,"evalbinop: bad type", e);
    }
    return e;
}

...
NODE *e;
{
    ...
    default:
	error(EINTNOD, "evalunop: bad op", e);
    }
    return e;
}

...
** WARNING: non-constant operands with side effects require special handling!
** For the time being, we just punt; later can mung expression into a
** comma-expr.
**
** Special cases:
...
NODE *e;
{
    ...
    */
#if NEW > 1
    if (sideffp(e->Nleft) || sideffp(e->Nright))
	return e;			/* Something has side effs, give up */
...
** WARNING: If the left-hand operand has side effects (eg is volatile)
**	then the op must be forced anyway even if nothing otherwise happens.
**
...
fi/=0	Warn	fi/=1	=> fi	fi/=(-1) => fi= -fi  ui/=log2(n) => ui>>=n
i%=0	Warn	i%=1	=> i=0	si%=(-1) => si=0     ui%=log2(n) => ui&=(n-1)
e+=0  => e	e+=1 => ++e	e+=-1 => --e
e-=0  => e	e-=1 => --e	e-=-1 => ++e
i<<=0 => i
i>>=0 => i
...
NODE *e;
{
...
#if NEW > 2
    if (sideffp(e->Nleft))	/* If destination expr has side-effs, */
	return e;		/* leave expr alone, sigh. */
	...
	case Q_ASDIV:			/* Check integral /= */
		if (icon == 0)
		    warn(EGEN, "Dividing by zero");	/* i/=0 => warn */
		else if (icon == 1)
		    return e->Nleft;	/* i/=1 => i */
	...
	case Q_ASMOD:			/* Check integral %= */
		if (icon == 0)
		    warn(EGEN, "Dividing by zero");	/* i%=0 => Warn */
		else if (icon == 1 || tissigned(e->Ntype)) {
...
NODE *e;
{
	...
	*/
	...
	default:
	    error(EINT,"evalcast: bad cast: %d", e->Ncast);
	    return e;
    }
...
NODE *n;
{
	...
	default:
	    error(EINTNOD, "edisc: non-expr", n);
	    break;
    }
    return n;
}
...
NODE *n;
{
	...
	default:
	    error(EINTNOD,"sideffp: bad op", n);
