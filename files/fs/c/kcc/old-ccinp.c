...
**		All changes after v.266, 7-Mar-1989
**	(c) Copyright Ken Harrenstien, SRI International 1985, 1986
**		All changes after v.64, 8-Aug-1985
**
...
_nextc()
{
	...
	*erptr++ = i;		/* Add to saved input line. */
	...
	fline--;			/* Account for unnecessary bump */
	tline--;			/* that happens when we */
...
skipcomm()
{
    ...
    saveolstop = eolstop;		/* Ensure comment doesn't end early */
    eolstop = NULL;
...
#define STDIOBUG 1
	...
	if (!(tmpfname == /*tmpnam(tmpfname)*/
		strcpy(tmpfname, asmtfile))) {	/* Use name from CCDATA */
...
cinclude()
{
	...
	** Note that 1st entry (if it exists) is default and is tried last;
	** it's OK if no default exists.
	*/
	if (nhfsypaths && (strncmp(f, "sys/", 4) == 0)
	  && (nhfsypaths <= 1
		|| !cinctry(nhfsypaths-1, hfsypaths+1, f2, f+4, &fp)))
	    cinctry(1, hfsypaths, f2, f+4, &fp);
	...
	*/
	if (!fp
	  && (nhfpaths <= 1 || !cinctry(nhfpaths-1, hfpaths+1, f2, f, &fp)))
	    cinctry(1, hfpaths, f2, f, &fp);
    }

    if (!fp)				/* If nothing worked, complain */
	error(EGEN, "Can't open include file, last tried \"%s\"", f2);
