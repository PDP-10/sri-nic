...
/* Common refs */
extern void vfun();
extern int ifun(), printf();

/* 28-Aug-89 Yet another bug in foldboth() (where else?) which was zapping
** a register that was supposed to stay around.
** Reported 28-Aug-89 by Peter Samson at SC (S.Nelson@Macbeth.Stanford.EDU)
*/
bugfb1()
{
	register int *x;
	...
	x = (int *)calloc(02000, 4);
	x[1] = -6;			/* these 8 statements are necessary */
	x[2] = -5;
	x[3] = -4;
	x[4] = -3;
	x[5] = -2;
	...
	x[7] = 0;
	x[8] = 1;
	x[9] = 0600020000405 | (int)x;	/* this one wins */
	x[10] = 0400100000410 | (int)x;	/* this one loses */
}

/* 29-Jun-89 yet another bug in foldboth(), which blew away the IBP
** here.  Fixed, which caused suboptimal ADJBP code, which inspired
** adding even more smarts to the expression evaluator.
** Reported 29-Jun-89 by Ben @ CSI.
*/
bugibp(buf)
char *buf;
{
...

/* 02-Feb-89  [KCC 608] Bug in CCOPT's adjstack(), it wasn't checking for
** things like ADD 1,17 and thus flushed ADJSPs without any correction.  Since
** hard to do corrections of this sort, code was fixed to check for
** unusual refs to 17 and punts the optimization if so.
** Reported 30-Jan-89 by Nelson H.F. Beebe.
*/
bugastk()
{
    float f[11];
    ...
    prob =  fst[patch] * f[ifun(x,0,10)] + f[ifun(x,0,10)];
}

...

/* 3-Jun-88 This bug caused CCTYPE's convbinary() to generate a double cast
** for the lefthand operand (from char to int to long) which confused the code
...

/* 3-Jun-88 This uncovered a bug in CCOUT's outptr() where it was just
** outputting the symbol part of a pointer initializer without checking
** for SC_ISTATIC indirection.
*/
...
long *clock;
{
    ...
    t = &ctmbuf;
    if (!ifun(0123,acs)) return(0);	/* something went wrong */
...
{
    char *cp;
    (bregfl ? (void)ifun() : (void)0);	/* Should ignore the 0 completely */
    (bregfl ? (*++cp = i) : ifun(i));	/* Discards implicit char->int cast */
}

...
{	char no[20];
	...
	if (!ifun(no, ptr->data.npg, &(ptr->data.count), &(ptr->data.type)))
		return (0);
	return 1;
}
#if 0
...

/* This may have been a bug at one time.  It stresses the code for
